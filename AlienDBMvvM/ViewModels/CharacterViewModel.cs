...
public class CharacterViewModel : ViewModelBase
{
        ...
        public ObservableCollection<CharacterModel> NewCharacters { get; set; } = new ();
        public ObservableCollection<string> AvailableSpecies { get; set; } = new();
        [Reactive] public ObservableCollection<CharacterModel> FilteredCharacters { get; set; }
        private string _selectedSpecies = "Wszystkie";
        public string SelectedSpecies
        {
            ...
            set
            {
                ...
                ApplyFilter();   // <-- TU JEST WYWOŁANIE
            }
        }


        public CharacterViewModel(string[] data)
        {
            this.MainCharacters = data;

            foreach (var characterName in MainCharacters)
            {
                string spiecies = "Brak Informacji";
                
                var match = Charaters.FirstOrDefault(c =>
                    ($"{c.Name} {c.Surname}".Trim().Contains(characterName, StringComparison.OrdinalIgnoreCase)) ||
                    ($"{c.Surname} {c.Name}".Trim().Contains(characterName, StringComparison.OrdinalIgnoreCase))
                );

                if (match != null)
                    spiecies = match.Spiecies;

                NewCharacters.Add(new CharacterModel()
                {
                    Name = characterName,
                    Spiecies = spiecies
                });
            ...
            FilteredCharacters = new ObservableCollection<CharacterModel>(NewCharacters);
        ...
        private void LoadAvailableSpecies()
        {
            ...
            AvailableSpecies.Add("Wszystkie");

            foreach (var s in NewCharacters
                         .Select(x => x.Spiecies)
                         .Distinct()
                         .OrderBy(x => x))
            {
        ...
        {
            if (SelectedSpecies == "Wszystkie")
            {
                FilteredCharacters = new ObservableCollection<CharacterModel>(NewCharacters);
            }
            else
            {
                FilteredCharacters = new ObservableCollection<CharacterModel>(
                    NewCharacters.Where(c => c.Spiecies == SelectedSpecies)
                );
            }
        }
